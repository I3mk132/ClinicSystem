...
def list_departments(db: Session = Depends(get_db)):
    return db.query(Department).filter(Department.is_active.is_(True)).order_by(Department.id).all()
...
def available_slots(doctor_id: int, target_date: date_type = Query(..., alias="date"), db: Session = Depends(get_db)):
...
def create_public_appointment(payload: PublicAppointmentCreate, db: Session = Depends(get_db)):
    ...
    # Find-or-create a lightweight patient record keyed by phone number.
    ...
    patient = db.query(User).filter(User.phone == payload.patient_phone).first()
    if not patient:
        patient = User(
...
def list_appointments_by_phone(phone: str = Query(..., min_length=5), db: Session = Depends(get_db)):
    patient = db.query(User).filter(User.phone == phone).first()
    if not patient:
        return []
    appointments = (
        db.query(Appointment)
...
def cancel_public_appointment(appointment_id: int, phone: str = Query(..., min_length=5), db: Session = Depends(get_db)):
    appointment = (
        db.query(Appointment)
        .options(joinedload(Appointment.patient), joinedload(Appointment.doctor), joinedload(Appointment.department))
